"""
LLM Analytics Report — sw-uatirl001
=====================================
Replicates:
  SELECT TENANT_ID, PROJECT_ID, SERVICE_NAME,
         count(1), sum(TOTAL_TOKENS)
  FROM   FACT_LLM_EVENTS
  WHERE  ENV = 'sw-uatirl001'
  GROUP  BY TENANT_ID, PROJECT_ID, SERVICE_NAME
  ORDER  BY 1,2,3;

Requires:  pip install requests

Usage:
  python llm_analytics_uatirl001.py               # show summary table + event ID count
  python llm_analytics_uatirl001.py --export      # save to CSV
  python llm_analytics_uatirl001.py --debug       # show raw API record + field mapping
"""
...
#  CONFIG
# ══════════════════════════════════════════════════════════════════════════════
URL     = "https://sw-stg-gsk.aeratechnology.com/aera/cortex/agent-memory/api/v1/llm-analytics/snapshot-raw"
TOKEN   = "1cfc464f365ca005c13840cfd573eece"
TENANTS = ["kmvz9eu70"]
...
def fetch_all():
    ...
    print(f"  Environment : sw-stg-gsk")
...
def export_csv(rows: list):
    fn = f"llm_uatirl001_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
...
def main():
    ...
    print(f"\n{'─'*60}")
    print(f"  LLM Analytics  —  sw-stg-gsk")
    print(f"{'─'*60}\n")
